...
from wc.kalshi.client import KalshiClient, API_PREFIX, LIVE_HOST
...
class KalshiClientV2(KalshiClient):
    ...
    def liquidity(market):
        """{volume, oi, ask_size, bid_size} for a market dict. The batch payload's
        `volume` is null; real fields are volume_fp / open_interest_fp /
        yes_ask_size_fp / yes_bid_size_fp. ask_size/bid_size = top-of-book depth,
        used to CAP paper fills realistically (no frictionless infinite fills)."""
        def num(f):
            v = market.get(f)
            try:
