...
KEY_FILE_DEFAULT  = paths.KEY_FILE
...
class KalshiClient:
    ...
    def _place_order_v2(self, ticker, side, count, price_dollars, label):
        """Place a Kalshi V2 event-market order (the legacy /portfolio/orders endpoint
        was deprecated → 410 Gone; orders now POST /portfolio/events/orders).
          side: 'bid' = buy YES, 'ask' = sell YES
          price_dollars: US-dollar fixed-point (0–1), NOT cents
        IOC (immediate-or-cancel): fills what it can now, cancels the rest — no resting
        orders linger, matching the old place-then-cancel behaviour. Returns
        (success, order_id, avg_fill_price_dollars)."""
        import uuid
        body = {
            "ticker":                     ticker,
            "client_order_id":            str(uuid.uuid4()),
            "side":                       side,                    # bid=buy YES / ask=sell YES
            "count":                      str(int(count)),         # fixed-point contracts
            "price":                      f"{price_dollars:.2f}",  # US DOLLARS fixed-point
        ...
        try:
            ...
            filled = float(order.get("fill_count", order.get("fill_count_fp", 0)) or 0)
            avg    = order.get("average_fill_price")
            fill_price = float(avg) if avg not in (None, "") else price_dollars
            print(f"  Kalshi {label} {oid} → filled {filled:g} @ ${fill_price:.2f}", flush=True)
            return (filled > 0), oid, fill_price      # IOC: leftover auto-cancelled
        except Exception as e:
            print(f"  [KALSHI ERROR] {label} failed: {e}", flush=True)
            return False, "error", 0.0

    def buy(self, ticker, count, yes_price_cents=None, dry_run=True):
        """Buy `count` YES contracts (marketable IOC at the ask). yes_price_cents is in
        CENTS (caller convention); converted to dollars for the V2 body. Returns
        (success, order_id, fill_price_dollars)."""
        ...
        print(f"  Kalshi: buy {count} YES @ {price}¢  (cost ≈${count * price / 100:.2f})", flush=True)
        if dry_run:
            print("  [DRY RUN] would submit IOC buy.", flush=True)
            return True, "dry-run", price / 100.0
        return self._place_order_v2(ticker, "bid", count, price / 100.0, "buy")
    ...
    def sell_limit(self, ticker, count, yes_price_cents, dry_run=True):
        ...
        return self._place_order_v2(ticker, "ask", count, actual_price / 100.0, "sell")
    ...
    def sell_market(self, ticker, count, dry_run=True):
        ...
        return self._place_order_v2(ticker, "ask", count, best / 100.0, "market-sell")
