...
STATE = os.path.join(BASE, "arena_v2_state")
TALLY = os.path.join(BASE, "logs", "arena_v2.jsonl")
CATS_FILE = os.path.join(BASE, "categories_v2.json")

POP_SIZE = 8                 # teams per category (4 seeds + 4 random at start)
MAX_OPEN = 12                # max open positions a team adds per category
MIN_CLOSED_FOR_CULL = 8      # a team needs this many resolved bets to be cullable
CULL_FRACTION = 0.25
STARTING_CASH = 200.0
...
def _new_team(cat, params, lineage, cycle, tid):
    return {"id": tid, "cat": cat, "lineage": lineage, "params": sv.clip(params),
            ...
            "fitness": 0.0, "born": cycle}


def _seed_population(cat, cycle, next_id):
    rng = random.Random(1000 + cycle + next_id[0])
    teams = []
...
def _evolve(teams_by_cat, cycle, next_id):
    rng = random.Random(7000 + cycle)
    summary = {}
    for cat, teams in teams_by_cat.items():
        eligible = [t for t in teams if t["n_closed"] >= MIN_CLOSED_FOR_CULL]
        if len(eligible) < 4:
            summary[cat] = "n/a (insufficient resolved)"
            continue
        ...
        for _ in range(n_cull):
            ...
            survivors.append(_new_team(cat, params, f"child<{pa['id']}x{pb['id']}",
                                       cycle, next_id[0]))
            next_id[0] += 1
        teams_by_cat[cat] = survivors
        summary[cat] = f"culled {n_cull}, bred {n_cull}"
    return summary
...
def cycle_once():
    ...
    e = _evolve(teams_by_cat, cycle, next_id)
        ...
        for m in client.list_markets_by_series(series, status=status):
            ...
            out.setdefault(ec, []).append(
                {"ticker": tk, "series": series, "sub": m.get("yes_sub_title", ""),
                 "result": 1 if r == "yes" else 0, "ko": ko})
    return out
...
def replay(max_games=None):
    """Reset, then replay every resolved WC game chronologically: price each leg
    off its pre-game candlestick quote, let teams bet, settle on the real result,
    feed the Brain Assistant, and evolve — to SEED the population with history."""
    reset()
    cats = active_cats()
    next_id = [0]
    ...
    # home/away + per-series settled legs
    names = {}
    for m in client.list_markets_by_series("KXWCGAME", status="settled"):
        ...
        if h:
            names[ec] = (h, a)
    # union of series we need (active cats + context for anchoring)
    ...
    for ec in games:
        ...
        for cat, teams in teams_by_cat.items():
            ...
            if resolved_for_brain:
                brains[cat].update_stacker(list(resolved_for_brain.values()))
        _evolve(teams_by_cat, cycle, next_id)
        if cycle % 5 == 0:
    ...
    for cat in cats:
        _save(os.path.join(STATE, f"teams_{cat}.json"), teams_by_cat[cat])
        _save(os.path.join(STATE, f"brain_{cat}.json"), brains[cat].weights)
    _save(os.path.join(STATE, "meta.json"), {"cycle": cycle, "next_id": next_id[0]})
