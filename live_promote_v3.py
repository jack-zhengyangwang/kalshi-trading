...
import strategy_v3 as s3
import markets_v2 as mv
import arena_v3 as a3
from live_promote_v2 import (_series_to_cat, _event_homeaway, _exit_decision,
                             _maintenance)

CAT = "game_lines"
SWITCHBOARD = os.path.join(BASE, "switchboard_v3.json")
STATE = os.path.join(BASE, a3.STATE_V3.split(os.sep)[-1], "promote_state_v3.json")
PLOG = os.path.join(BASE, "logs", "promote_v3.jsonl")
...
def _member_open_exposure(cat, team, real_open):
    ...
    path = os.path.join(os.path.dirname(__file__), "logs", "promote_v3.jsonl")
